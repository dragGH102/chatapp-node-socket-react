...
describe('handleMessageUtility', () => {
    ...
    it('if not a command, should add the message to messages and messageIds', () => {
        const state = initialState;
        const message = generateMessage('me');
        const messageWithType = _.cloneDeep(message);
        messageWithType.type ='plain';
        console.log(initialState);
        const result = handleMessageUtility(state, message, socket);
        expect(result.messages[0]).toEqual(message);
        expect(result.messageIds[0]).toBe(message.id);
    });
});
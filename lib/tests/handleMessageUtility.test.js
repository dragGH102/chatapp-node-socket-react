...
describe('handleMessageUtility', () => {
    ...
    describe('plain message', () => {
        ...
        it('if message with command \'think\', proper style and content shall be set', () => {
            ...
            messageWithType.args = ['A', 'message'];

            const resultThink = handleMessageUtility(state, messageWithType, socket);
            const resultMessage = resultThink.messages[resultThink.messages.length -1];
            expect(resultMessage.css.color).toEqual('#2f4f4f');
            expect(resultMessage.content).toEqual('A message');
        });

        it('if message with command \'highlight\', proper style and content shall be set', () => {
            messageWithType.type ='highlight';
            messageWithType.args = ['A', 'message'];

            const resultThink = handleMessageUtility(state, messageWithType, socket);
            const resultMessage = resultThink.messages[resultThink.messages.length -1];
            expect(resultMessage.css.fontSize).toEqual('1.1em');
            expect(resultMessage.css.filter).toEqual('brightness(90%)');
            expect(resultMessage.content).toEqual('A message');
        });
    });

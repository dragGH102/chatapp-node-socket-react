...
export const handleMessageUtility = (state, message, socket) => {
    ...
    if (message.type === 'oops') {
        ...
        if (lastMessageId) {
            const lastMessage = newState.messages.findIndex((message) => message.id === lastMessageId);
            lastMessage.css.fontSize = '1.1em';
            // better than css filters for cross-browser support
            lastMessage.css.color = 'hsl(0, 100%, 10%)'; //
        }
    }
    else if (message.type === 'plain' || message.type === 'think') {
